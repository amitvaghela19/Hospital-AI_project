...
def get_champion_pipeline():
    ...
    if not path.exists():
        _pipe_error = (
            ...
            "python scripts/train_advanced_chat_artifacts.py"
        )
        _pipe_loaded = True
...
def artifact_status() -> dict[str, dict]:
    ...
    return {
        ...
        "shadow_tri_ensemble": {
            ...
            "detail": "Present" if shadow_path.exists() else "Run train_advanced_chat_artifacts.py",
        },
        "rnn_primary": {
            "ok": rnn_path.exists(),
            "detail": "Present" if rnn_path.exists() else "Run train_advanced_chat_artifacts.py",
        },
        "gold_features": {
            "ok": gold.exists(),
