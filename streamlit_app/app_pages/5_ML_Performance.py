...
with st.expander("How to read these charts", expanded=False):
    st.markdown(
        ...
        "### Model landscape (recall vs precision)\n"
        "- Each bubble is one trained model (color = model; **size ≈ ROC AUC**).\n"
        "- **Higher** = catch more true readmissions (recall). **Further right** = fewer "
        "false alarms (precision).\n"
        "- The **star + callout** is the deployed champion. We prioritized **recall**, so "
        "precision sits lower on purpose.\n"
        "- Dotted lines are the group medians; the teal band marks the higher-recall region.\n\n"
        "### Model ranking\n"
