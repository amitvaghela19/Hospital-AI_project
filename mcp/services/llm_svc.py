...
from mcp.services import redis_svc
...
def ollama_phrase_facts(facts: dict) -> tuple[str | None, str | None]:
    ...
    cache_key = redis_svc.llm_cache_key("phrase", model, facts)
    cached = redis_svc.cache_get(cache_key)
    if isinstance(cached, dict) and cached.get("text"):
        return str(cached["text"]), cached.get("model") or model
    ...
    text, used = llm_generate(prompt)
    if text:
        redis_svc.cache_set(cache_key, {"text": text, "model": used or model}, ttl_seconds=3600)
    return text, used
...
def ollama_format_chat(facts: dict) -> tuple[str | None, str | None]:
    ...
    """
    try:
        from streamlit_app.llm_provider import get_provider_mode

        if get_provider_mode() == "none":
            return None, None
    except Exception:
        pass
    if get_provider_mode_safe_ollama_only() and ollama_health().get("status") != "ok":
        return None, None

    model = _primary_model()
    cache_key = redis_svc.llm_cache_key("format", model, facts)
    cached = redis_svc.cache_get(cache_key)
    if isinstance(cached, dict) and cached.get("text"):
        return str(cached["text"]), cached.get("model") or model
    ...
    text, used = llm_generate(prompt, timeout_s=30)
    if text:
        redis_svc.cache_set(cache_key, {"text": text, "model": used or model}, ttl_seconds=3600)
    return text, used
...
def get_provider_mode_safe_ollama_only() -> bool:
    try:
        from streamlit_app.llm_provider import get_provider_mode

        return get_provider_mode() == "ollama"
    except Exception:
        return True


def ollama_generate_sql(prompt: str) -> str | None:
    """Generate an SQLite SELECT using the active LLM; fail fast if Ollama is down."""
    ...
    try:
        ...
        if mode == "none":
            return None
        if mode == "ollama" and ollama_health().get("status") != "ok":
            print("DEBUG ollama_generate_sql: Ollama unhealthy — skipping generate")
            return None
        if mode == "custom_api" and not custom_provider_configured():
            return None
    except Exception:
    ...
    # Prefer fallback (often smaller / non-reasoning) first for SQL latency, then primary once.
    fallback = _fallback_model()
    primary = _primary_model()
    models: list[str] = []
    for m in (fallback, primary):
        if m and m not in models:
            models.append(m)
    ans = None
    for model in models:
        ans, _ = llm_generate(full_prompt, model=model, timeout_s=60)
        if ans:
            break
    if ans:
        import re
